        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {skills.map((category, i) => (
            <motion.div
              ...
              className="bg-[#FAFAF8] border border-zinc-100 rounded-2xl p-6 hover:border-zinc-200 transition-colors duration-200"
            >
              ...
              <div className="flex flex-wrap gap-2">
                {category.items.map((item) => (
                  <span
                    key={item.name}
                    className={`text-sm font-mono px-3 py-1.5 rounded-full border transition-colors duration-150 ${
                      item.highlight
